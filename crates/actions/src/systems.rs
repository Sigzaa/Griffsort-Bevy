...
pub fn collect_actions<
    Sel: Component,
    Keys: std::hash::Hash + Eq + Sync + Send + Clone + Debug + 'static,
>(
    ...
    for mut actions in &mut q_selected
    {
        for key in bindings.keyboard_bindings.keys()
        {
            if keyboard.just_pressed(*key)
            {
                actions
                    .pressed
                    .insert(bindings.keyboard_bindings.get(key).unwrap().clone());
                actions
                    .just_pressed
                    .insert(bindings.keyboard_bindings.get(key).unwrap().clone());
            }
        ...
        for key in bindings.mouse_bindings.keys()
        {
            if mouse.just_pressed(*key)
            {
                actions
                    .pressed
                    .insert(bindings.mouse_bindings.get(key).unwrap().clone());
                actions
                    .just_pressed
                    .insert(bindings.mouse_bindings.get(key).unwrap().clone());
            }
...
pub(crate) fn watch_for_changes<
    Keys: std::hash::Hash + Eq + Sync + Send + Clone + Debug + 'static + Serialize,
>(
    bindings: Res<Keybindings<Keys>>,
...
pub(crate) fn load_bindings<
    Keys: for<'de> Deserialize<'de> + Eq + std::hash::Hash + Send + Sync + Clone + Debug + 'static,
>(
    ...
    match (is_config_exist, is_default_exist)
    {
        ...
        (false, false) =>
        {
            ...
            panic!(
                "You have to fill default config file ({}) first",
                path.default_path
            );
        }
